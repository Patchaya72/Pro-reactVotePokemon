...
function ProfileRank_Page() {
  const services = new Service();
  
  const [searchParams] = useSearchParams();
  const ImgIDString = Number(searchParams.get("ImgID"));
  ...
  useEffect(() => {
    const loadDataAsync = async () => {
      const res = await services.getImgById(ImgIDString);
      const Imgs: PicturePostRequest[] = res;
      console.log(Imgs);

      const res1 = await services.getUserById(Imgs[0].Uid);
      const re: UsersGetRespose[] = res1;
      console.log(re);
      setUser(re);
...
export default ProfileRank_Page;
